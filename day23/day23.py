...
if __name__ == '__main__':
    ...
    # limits = find_limits(bots)
    # print(limits)
    #
    # # matrix = [[[0] * limits.max_x for _ in range(limits.min_y, limits.max_y)] for _ in range(limits.min_z, limits.max_z)]
    # #
    # # matrix = np.zeros(shape=(limits.max_x, limits.max_y, limits.max_z))
    #
    # max_bots_in_range = 1
    # max_ = []
    # for x in range(limits.min_x, limits.max_x):
    #     for y in range(limits.min_y, limits.max_y):
    #         for z in range(limits.min_z, limits.max_z):
    #             bots_in_range = 0
    #             for bot in bots:
    #                 if is_in_range(x, y, z, bot):
    #                     bots_in_range += 1
    #
    #             if bots_in_range >= max_bots_in_range:
    #                 max_ = [x, y, z]
    #                 max_bots_in_range = bots_in_range
    #                 print("Current max location:", max_)
    #                 print("Current max:", max_bots_in_range)
    #
    # print(max_)
    # # Numpy Where
    # # max = np.where(matrix == matrix.max())
    #
    # # Numpy Where as single array
    # # max = np.hstack(np.where(matrix == matrix.max()))
    #
    # # Numpy flatten and unravel
    # # indices = np.argpartition(matrix.flatten(), -2)[-1:]
    # # print(np.vstack(np.unravel_index(indices, matrix.shape)).T)
    #
    # print("Manhattan distance from 0,0,0 to location:", max_[0] + max_[1] + max_[2])
